...
namespace PV178.Homeworks.HW02.Tests.Tests
{
    ...
    public class ControlUnitTests
    {
        ...
        public void GetStockFromCoordinates_ValidCoordinatesWithNonEmptyStock_ReturnsCorrespondingStock()
        {
            ...
            Assert.AreEqual(new Stock(new Product("Mattoni 250ml", 25), 5).ToString(), stock.ToString(), "GetStockFromCoordinates(...) - for given valid coordinates the resulted stock does not equal the expected one");
        }
        ...
        public void GetStocksDictionary_AllInitializedStocks_VerifyEqualityByExpectedOutput()
        {
            ...
            {
                if (!dictionaryToCheck.ContainsKey(keyValuePair.Key) || !dictionaryToCheck[keyValuePair.Key].ToString().Equals(keyValuePair.Value.ToString()))
                {
